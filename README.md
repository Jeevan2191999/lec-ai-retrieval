...
| Hybrid | 0.5      | 0.27 | 11.3ms      |

### Which Config is Best?
Dense wins on Recall@5 (0.8), finds the right document 80% of
the time. For production where finding the document matters most,
Dense is the best config.

BM25 wins on MRR (0.26) and speed (0.62ms), when it finds the
right doc it ranks it higher. 2700x faster than Dense.
...
acronyms or specific model names return semantically related
but not exact matches. BM25 handles these better due to exact
term matching.
